...
export interface CanonicalMemoryStoreFactoryV1 {
  ...
  // STORE_UNAVAILABLE) when a live holder already exists. The acquired store carries this build's in-process
  // provenance mark, and its readiness() declares this build's compiled adapter identity (§5.9).
...
export interface OperationalCapabilityReceiptV1 {
  ...
  // (new in this amendment) OPTIONAL declared adapter identity. Omitted by non-production stores (backend
  // "memory" and embedded-local test fakes). For a production store the graphify-owned fenced-store factory
  // stamps these three fields from its module's compiled identity; the engine compares them to its own compiled
  // identity and pairs that with an in-process provenance mark (§5.9). There is NO signature. All three present
  // or all absent.
...
*Threat scope (stated precisely).* This attestation guards against a **misconfigured or naive store** that would falsely report fencing — an adapter/version mismatch, a stale or wrong build, or a store that never took the fence. It does **not**, and cannot, guard against a **malicious in-process host**: that host controls the process and any in-process key, and can therefore forge any in-process assertion. The trust boundary here is deployment topology, not cryptography — the host is inside the trust boundary by construction. No unproven crypto trust root is claimed; there is **no signature and no key**. Within that scope, an **in-process provenance mark** (the graphify-owned fenced-store factory stamps each store it builds, after taking the fence) proves that a store came from this graphify build's factory, and the store's **declared adapter identity** is compared against this build's **compiled identity** — a store-independent source — so an adapter/version/build mismatch and a store that never took the fence are **detected**, not authenticated against a hostile host.

*Single admission path (in-process provenance).* A production store is admitted for fencing-dependent operations only when both hold: **(1) provenance** — the store was built by THIS graphify build's fenced-store factory, proven by an in-process mark the factory applies after taking the fence. This mark is the only detector of a store that never took the fence, or of a duplicate package (a second module instance whose stores are not members of this instance's factory registry); a copyable string cannot prove it. **(2) declared identity** — the store's `adapter_id`/`adapter_version`/`adapter_build_digest` equal this build's **compiled** adapter identity, a source of truth **independent of the store and of host configuration** (a host that misconfigures the store cannot also make it self-consistent against a compiled constant). The **engine** (`createMemoryPortV2`) performs both checks before any fencing-dependent operation and never delegates to the store's own `readiness()`. There is **no signature and no key** — the trust boundary is deployment topology, not cryptography. A store that is not a member of this build's factory registry, or whose declared identity does not match, is refused exactly as an unfenced store. This binds **embedded-local production SQLite**, the sole operational mode (§12). **`external-host`** is design-reserved: an in-process mark does not cross a process boundary, so a cross-process store would instead be compared against an expected identity held by the engine, and its "never took the fence" property would rest on deployment-topology trust; no cross-process mechanism is defined here.

*Optionality.* The declared adapter identity is optional on the receipt: non-production stores — `backend: "memory"` and embedded-local test fakes — omit it and carry no provenance mark, and are therefore admitted only where production fencing is not required.
