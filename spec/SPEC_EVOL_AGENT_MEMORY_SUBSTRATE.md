...
The flock is an open-file-description lock, so the kernel releases it when the **last fd** on that description closes — a graceful `close()` or process death — which is why a crash never strands the fence (*release-after-close*), and why the fenced inode must carry exactly one name: an aliased inode would outlive the unlink of the canonical path and could be relinked into place after release, reintroducing a database the fence believed retired. Kernel release after process death permits a new owner, which increments the epoch before any application write. Busy timeout is only a fuse; it is not ownership. Active readers have finite revocable leases tied to the epoch. Ranking and backup use a SQLite backup-API detached copy pinned to a high-water cursor; they never hold the active WAL. WAL/checkpoint operations run only through the fenced owner.
