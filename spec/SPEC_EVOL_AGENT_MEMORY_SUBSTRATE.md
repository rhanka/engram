...
- **(f)** if the injected provider throws instead of returning a `Result`, `admin` surfaces `POLICY_UNAVAILABLE` (the admin provider is the authority of the `authorization_epoch`, §5.10) — the engine keeps `admin` total on its `Result` contract, fail-closed with `retryable: false`; it never maps a throw to `UNAUTHORIZED` or sets `denial: true` (an exception declared no authorization decision, so mapping it to a denial would re-introduce the inference (d) excludes), and never surfaces the provider's raw error text (D2 minimal redaction: it could carry a credential reference, path, or DSN).
