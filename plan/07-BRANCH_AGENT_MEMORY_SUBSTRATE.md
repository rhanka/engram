...
- [ ] Implement bounded current projection, deterministic size fixture, complete local checkpoints, portable logical backup/restore, projection invalidation cascade, the Postgres canonical adapter, and cross-backend state/receipt parity. (L6a — bounded projection, size fixture, logical backup/restore, cascade — landed backend-agnostic; L6b — Postgres adapter + parity — landed. DELETE+full re-INSERT write-amplification ceiling recorded in spec §8/§9 D9 per storage-lane review O3; not re-architected here.)
...
- [x] RED→green (L6b): `tests/canonical-memory-store.parity.test.ts > SQLite and Postgres produce identical canonical state digests for the lifecycle corpus` (real Docker Postgres 16 + 17; digests folded from a reopened store so persistence divergence is caught — verified non-vacuous against injected outbox/journal corruption; gated to `it.skip` when Docker/images are absent).
- [x] RED→green (L6b): `tests/postgres-memory-store.native.test.ts > promotion is atomic and generation mismatch fails before mutation` (real Docker Postgres 16 + 17; failpoint rollback leaves blob/journal/state/lexical/outbox unchanged, and a superseded generation refuses `FENCE_LOST` before any mutating statement).
