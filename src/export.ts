...
export function backupIfProtected(outDir: string): string | null {
  ...
  try {
    ...
    if (copied > 0) {
      console.log(`[engram] backed up ${reason} graph (${copied} files) → ${basename(backupDir)}/`);
    }
    return backupDir;
  } catch (err) {
    console.warn(
      `[engram] warning: backup failed (${err instanceof Error ? err.message : err}) — continuing with overwrite`,
    );
    return null;
  }
}
...
function buildFreshnessMetadata(outputPath: string): { built_from_commit?: string } {
  ...
  if (stateDir !== ".engram" && stateDir !== ".graphify" && stateDir !== "graphify-out") return {};
  ...
  if (!forceWrite) {
    try {
      ...
      if (existingNodeCount > nodes.length) {
        console.warn(
          `[engram] WARNING: new graph has ${nodes.length} nodes but existing graph.json has ` +
...
export function toCypher(G: Graph, outputPath: string): void {
  const lines: string[] = ["// Neo4j Cypher import - generated by the engram skill", ""];
 ...
 */
/** Current (Engram) Spanner export table names — writers emit these. */
export const SPANNER_EXPORT_NODE_TABLE = "engram_nodes";
export const SPANNER_EXPORT_EDGE_TABLE = "engram_edges";
export const SPANNER_EXPORT_PROPERTY_GRAPH = "engram";

export function spannerDdlLines(): string[] {
  return [
    "-- Spanner DDL generated by engram",
    "-- GoogleSQL dialect · Cloud Spanner Property Graph",
    "",
    `CREATE TABLE ${SPANNER_EXPORT_NODE_TABLE} (`,
    ...
    ") PRIMARY KEY (id);",
    "",
    `CREATE TABLE ${SPANNER_EXPORT_EDGE_TABLE} (`,
    ...
    `CREATE PROPERTY GRAPH ${SPANNER_EXPORT_PROPERTY_GRAPH}`,
    "  NODE TABLES (",
    `    ${SPANNER_EXPORT_NODE_TABLE}`,
    "      KEY (id)",
    "      LABEL node",
    "      PROPERTIES (label, node_type, community)",
    "  )",
    "  EDGE TABLES (",
    `    ${SPANNER_EXPORT_EDGE_TABLE}`,
    "      KEY (source_id, target_id, relation)",
    `      SOURCE KEY (source_id) REFERENCES ${SPANNER_EXPORT_NODE_TABLE} (id)`,
    `      DESTINATION KEY (target_id) REFERENCES ${SPANNER_EXPORT_NODE_TABLE} (id)`,
    "      LABEL edge",
...
export function toSpanner(G: Graph, outputDir: string): void {
  ...
  // -------------------------------------------------------------------------
  const dml: string[] = [
    "-- Spanner DML generated by engram",
  ...
  G.forEachNode((nodeId, attrs) => {
    ...
    dml.push(
      `INSERT OR UPDATE INTO ${SPANNER_EXPORT_NODE_TABLE} (id, label, node_type, community, props) ` +
  ...
  G.forEachEdge((_edge, edgeAttrs, u, v) => {
    ...
    dml.push(
      `INSERT OR UPDATE INTO ${SPANNER_EXPORT_EDGE_TABLE} (source_id, target_id, relation, confidence, props) ` +
