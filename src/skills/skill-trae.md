...
/engram migrate-state --dry-run                    # plan graphify-out -> .graphify migration
...
This writes a `## graphify` section to the local `AGENTS.md` that instructs Trae to check the graph before answering codebase questions and rebuild it after code changes. No manual `/engram` needed in future sessions.
