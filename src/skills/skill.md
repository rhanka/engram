...
```bash
engram detect "INPUT_PATH" --scope auto --out .graphify/.graphify_detect.json
```
...
>
> 1. `engram update .` (emits `.graphify/description-instructions/` + `.graphify/label-instructions/`).
...
engram wiki describe --graph .graphify/graph.json --mode assistant --targets all --max-nodes 0 --max-communities 0
# or, for direct mode:
...
engram export wiki --graph .graphify/graph.json
engram export wiki --graph .graphify/graph.json --descriptions .graphify/wiki/descriptions.json
engram export obsidian --graph .graphify/graph.json --descriptions .graphify/wiki/descriptions.json
```
...
```json
{
  "mcpServers": {
    "engram": {
      "command": "npx",
      "args": ["engram", "serve", "/absolute/path/to/.graphify/graph.json"]
    }
  }
}
```
