...
function communityArticle(
  ...
  lines.push("---", "", "*Part of the engram knowledge wiki. See [[index]] to navigate.*");
  return lines.join("\n");
}
...
function indexMd(
  ...
  const lines: string[] = [
    ...
    "",
    "> Auto-generated by Engram. Start here — read community articles for context, then drill into god nodes for detail.",
    "",
  ...
  if (communityMap.size === 0) {
    throw new Error(
      "all community node IDs are stale — none exist in the graph. " +
        "Re-run `engram extract .` to regenerate .graphify_analysis.json.",
    );
  }
